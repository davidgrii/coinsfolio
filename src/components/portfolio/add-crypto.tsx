    ...
    >
      ...
      >
        {selectedCrypto ? (
          <div
            className={cn(
              'relative flex items-center justify-between w-full px-3 py-4 h-[48px] !bg-neutral-04 rounded-xl',
              ...
              after={
                searchValue ? (
                  ...
                  <Icons.question
                    onClick={() =>
                      alert(
                        'Search for a coin by name or symbol. Example: Bitcoin, BTC',
                      )
                    }
              ...
              <div className='absolute top-14 mb-2 w-full z-10 border-neutral-04 border rounded-xl overflow-hidden'>
                <div className='bg-base-background rounded-xl shadow-md max-h-[35dvh] overflow-y-auto'>
                  {isCryptosLoading || isCryptosFetching
                    ? new Array(8)
                        .fill(null)
                        .map((_, index) => <SearchCryptoSkeleton key={index} />)
                    : cryptos?.map((crypto) => {
